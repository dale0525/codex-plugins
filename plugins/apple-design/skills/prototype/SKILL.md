---
name: prototype
description: Build multiple genuinely different versions of a UI piece you describe, rendered behind a visual picker so you can flip through them live and promote the one that feels right. Only runs when explicitly invoked; it does not trigger on its own. Only runs when explicitly invoked to build and compare UI variants; it does not trigger for ordinary UI implementation or review.
---
