...
description: |
  ...
  仅在用户明确要求去除 AI 痕迹或进行人性化改写时使用；在上游网文编排明确传入 `orchestrated_fiction_edit` 时可执行受限的表达层扫描；不得新增原文没有的事实、经历、观点或题外内容，也不替代结构、人物、场景和连续性修订。
---
...
当用户明确要求去除 AI 痕迹或进行人性化改写时：

- 只编辑表达，不新增原文没有的事实、功能、反馈、经历、立场或题外内容。
- 保留原意、信息边界和可核验主张；原文信息不足时指出缺口，不自行补全。
- 不改动作品结构、人物选择、场景设计或连续性；这些问题应交给对应的创作或修订技能。

## 编排模式

`orchestrated_fiction_edit` 只由上游网文创作编排显式传入，不改变本技能的显式用户触发策略。该模式的目标是清理表达层噪声，同时把叙事决定留给 provider 和对应创作技能：

- 每次扫描都重新检查候选文本的全部模式，再决定是否有必要动笔；没有表达层收益就原样返回。
- 编辑结果必须是仅语义等价的表达层最小修改：不新增或删除事实、事件、场景、动机、信息权限、关系状态、代价或类型承诺，不改变 POV、叙事距离或连续性。
- 保留有功能的排比/反复/破折号/金句/残句；只有确认它不承担信息、人物、节奏、伏笔或情绪功能，且存在等价替换时才可改。
- 若需新增场景或改变言语行为，退回 `provider`；若修复需要改变选择/后果，也立即停止该处修改，说明触发的上游决定，不要用人性化改写代替创作。
- 交付候选文本、表达层改动摘要和未处理项；没有可安全修改的片段时明确报告保持原文。

1. **识别 AI 模式** - 扫描下面列出的模式
2. **按模式处理** - 显式用户模式可重写问题片段；编排模式只修改能安全等价替换的表达层命中
3. **保留含义** - 保持核心信息完整
4. **维持语调** - 匹配预期的语气（正式、随意、技术等）
5. **保留作者声音** - 只在原文已有语气和立场的范围内恢复自然表达
...
5. **删除金句（显式用户模式）** - 如果听起来像无功能或宣传化的可引用语句，重写它；编排模式先保留有叙事功能的金句
...
**保留已有观点。** 原文有明确立场时，让它表达得更自然；原文只陈述事实时，不替作者新增态度。

**变化节奏。** 短促有力的句子。然后是需要时间慢慢展开的长句。混合使用。

**保留已有复杂性。** 原文已经表达不确定或矛盾感受时，让这种复杂性自然呈现；没有相关信息时不要自行添加感受。

**谨慎使用"我"。** 只有原文已有第一人称主体或用户明确提供相关经历时才保留；不要虚构作者经历。

**允许自然的不齐整。** 可以调整句式和节奏，但不要为了显得像人而添加跑题内容或改变信息结构。

**对已有感受要具体。** 原文提供了具体场景或感受时，可以保留并改得更清楚；没有相关细节时不要虚构场景。
...
**改写后：**
> 加泰罗尼亚统计局于 1989 年成立。这是在西班牙分散行政职能、加强区域治理的背景下进行的。
...
**改写后：**
> 她的观点曾被《纽约时报》、BBC、《金融时报》和《印度教徒报》引用，社交媒体粉丝超过 50 万。
...
**改写后：**
> 寺庙使用蓝色、绿色和金色，与当地的蓝帽花、墨西哥湾和其他景观相呼应。
...
**改写后：**
> Alamata Raya Kobo 是埃塞俄比亚贡德尔地区的一座城镇，有文化遗产和自然景观。
...
**改写后：**
> 浩来河因其独特特征引起研究人员和保护主义者关注；其生态作用需要具体来源支持。
...
**改写后：**
> Gallery 825 是 LAAA 的当代艺术展览空间，有四个独立空间，面积超过 3000 平方英尺。
...
**改写后：**
> 该公司可能成立于 20 世纪 90 年代，但现有资料不足以确定具体年份。
...
- ✓ **连续三个句子长度相同？** 打断其中一个
- ✓ **段落以简洁的单行结尾？** 变换结尾方式
- ✓ **揭示前有破折号？** 显式用户模式可检查是否过量；编排模式先确认它没有叙事功能再动
- ✓ **解释隐喻或比喻？** 相信读者能理解
- ✓ **使用了"此外""然而"等连接词？** 考虑删除
- ✓ **三段式列举？** 改为两项或四项
...
下面的模式清单和评分只提供诊断线索，不覆盖编排模式的边界。编排模式中，只有不承担信息、人物、节奏、伏笔或情绪功能且存在语义等价替换的命中才算可修改问题；否则保留原文。

1. 仔细阅读输入文本
2. 识别上述所有模式的实例
3. 只重写当前模式允许安全处理的问题片段；没有可安全修改的片段就保持原文
4. 确保修订后的文本：
   - 大声朗读时听起来自然
   - 自然地改变句子结构
   - 只使用原文已有的具体细节；不把模糊主张改写成虚构事实
   - 为上下文保持适当的语气
   - 适当时使用简单的结构（是/有）
5. 呈现人性化版本
...
评分仅供显式用户模式按需自检；编排模式不得用分数迫使无明确表达层收益的文本发生变化。
...
**改写后（人性化）：**
> 这次软件更新改进了用户完成任务的体验。至于具体功能、效率变化和用户反馈，原文没有提供，改写时不能自行补充。
