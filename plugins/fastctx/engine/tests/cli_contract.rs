// Modified by dale0525/codex-plugins for the compact managed-marker contract.
mod common;
...
fn apply_status_and_unapply_cover_both_shell_states() {
    for fastshell in [false, true] {
        ...
        assert_eq!(
            agents,
            format!("{}\n", fastctx::control::agents::AGENTS_SECTION)
        );
...
fn apply_migrates_owned_three_server_config_and_legacy_agents_blocks_atomically() {
    ...
    assert_eq!(agents.matches("<!-- fastctx:begin -->").count(), 1);
    assert!(
        agents.contains(fastctx::control::agents::AGENTS_SECTION),
        "{agents}"
    );
