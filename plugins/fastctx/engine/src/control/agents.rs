...
//!
//! Modified by dale0525/codex-plugins for its compact managed-marker contract.
...
pub fn section(_fastshell_enabled: bool) -> String {
    AGENTS_SECTION.to_string()
}
...
mod tests {
    ...
    #[test]
    fn reapply_replaces_legacy_long_guidance_with_the_compact_contract() {
        let original = b"before\n\n<!-- fastctx:begin -->\n### Bulk edits and moving code\nUse mcp__fastctx__copy then mcp__fastctx__paste.\n<!-- fastctx:end -->\nafter\n";
        let applied = apply_section(original).unwrap();
        let source = std::str::from_utf8(&applied).unwrap();
        assert!(source.contains(AGENTS_SECTION));
