...
//!
//! Modified by dale0525/codex-plugins to disable upstream self-updates in plugin builds.
...
const CODEX_PLUGIN_DISTRIBUTION: &str = "codex-plugin";
...
fn detect_install_channel(
    ...
    // The Codex plugin owns distribution and updates atomically through its reviewed
    // release metadata. It must never use the upstream self-update channels.
    if option_env!("FASTCTX_DISTRIBUTION") == Some(CODEX_PLUGIN_DISTRIBUTION) {
